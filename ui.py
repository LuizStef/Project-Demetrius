...
class DemetriusUI(QMainWindow):
    ...
    def _open_settings(self):
        ...
        if dlg.exec():
            THEME["font_size"] = int(dlg.font_size.currentText())
            THEME["accent"]    = dlg._accent
            try:
                self.demetrius.set_mood(dlg.mood.currentText())
                self.mood_label.setText(dlg.mood.currentText())
            except:
                pass
            self._apply_theme()
